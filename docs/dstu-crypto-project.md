...
| `crypto_box` | `hazmat::dstu9041` | **`hazmat` primitive implemented, `l(p)=256`/E256/1 only** (`docs/TASKS.md` T-177, D-47's "ship the recommended curve first" precedent) — `message`/`fp256`/`curve256`/`encryption` submodules, verified against the standard's own Додаток Г worked example (the sole oracle for this primitive, see `docs/ORACLES.md`). No high-level `crypto_box`-shaped wrapper yet; `l(p)=384/512/768` also not yet done. |
