...
| `crypto_box` | `hazmat::dstu9041` | **Done** (`docs/TASKS.md` T-178, D-169) — `dstu_core::crypto_box::{seal,open,SecretKey,PublicKey}`, hybrid via KDF over the `hazmat::dstu9041` `l(p)=256`/E256/1 primitive (T-177, D-47's "ship the recommended curve first" precedent), plus a `uacrypt box-keygen`/`box-pubkey`/`box-seal`/`box-open` CLI surface. `l(p)=384/512/768` still not done (T-182). |
