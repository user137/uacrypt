...
- **Annex A** (the standard's own DSTU 28147-based RNG) — this project uses `d`/`e` as
  caller-supplied parameters at the `hazmat` layer (see `docs/DECISIONS.md` D-04: OS `getrandom` is the
...
- Curve domain-parameter tables beyond m=163 (`DSTU4145NamedCurves.java`, Annex Г's recommended
  ...
  core above. `m=257` specifically is now implemented (`crypto_sign257`, `docs/TASKS.md` T-199,
  `docs/DECISIONS.md` D-185/D-186), not just enumerated by BC — real Diia-issued certificates, not
  just the standard allowing it, motivated pulling the domain parameters from them.
