...
- **Corrected stale bullet (this file previously contradicted itself here):** `t`/`C` in Додаток
  ...
  same day as T-174's first pass, just not reflected in this list until now. Додаток Г.3
  (`l(p)=512`) is now verified too (T-192, D-180) - `Q`/`R`/`T`/`kappa`/`H` all match the document's
  own printed hex exactly, `t`/`C` match to within a 2-hex-digit discrepancy attributed to the same
  printing-erratum pattern already documented for Додаток Г.1's own `t` (`g1-worked-example.json`'s
  `t_ciphertext_note`). What's still unverified is Додаток Г.2 (`l(p)=384`) - present in the scan
  (curve params, `Q`/`R`/`T` etc.) but its own `t`/`C` can't be checked yet regardless
  (`hazmat::kalyna_kw_p` doesn't exist, see below).
