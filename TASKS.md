...
- [x] **Blocker lifted 2026-07-22 (D-15), not resolved:** no official vectors exist yet (still
      priced/pending the info request), but "gray" vectors — generated by running
      `oracles/strumok-dstu8845/` itself, **not official, not independent verification, a
      regression anchor only** — now exist at
      `crates/dstu-core/tests/vectors/strumok/gray/keystream-{256,512}.json` (generator:
      `tests/oracle-harness/strumok-gray-generator/generate_gray_vectors.c`). This unblocks
      test-first implementation; it does not close the real verification gap — see `DECISIONS.md`
      D-15 for exactly what these vectors do and don't prove, and the follow-up required once real
      vectors ever exist.
- [ ] Implement Strumok test-first against the gray vectors above, structurally cross-checked
      against `outspace/dstu8845`'s source per the existing pseudocode doc. Any status line for
      this work must say "regression-anchored, not independently verified" — never
