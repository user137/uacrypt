...
//! cross the Ruby boundary as plain `String` (binary), matching every other wrapper in this crate.
...
fn secret_key_from_bytes(ruby: &Ruby, bytes: &[u8]) -> Result<SecretKey, Error> {
    let e = to_array::<64>(ruby, bytes, "secret_key")?;
...
fn public_key_from_bytes(ruby: &Ruby, bytes: &[u8]) -> Result<PublicKey, Error> {
    let x = to_array::<64>(ruby, bytes, "public_key")?;
...
pub fn box512_keygen(ruby: &Ruby) -> Result<RString, Error> {
...
/// compression).
pub fn box512_public_key(ruby: &Ruby, secret_key: RString) -> Result<RString, Error> {
...
/// Decrypts `sealed` (as produced by [`box512_seal`]) under `secret_key`. Raises `DstuCore::Error`
/// if authentication fails (wrong key, or any tampered wire segment - deliberately not
/// distinguished further, see `dstu_core::crypto_box512::OpenError`'s own doc comment) or `sealed`
/// is too short to be valid.
pub fn box512_open(ruby: &Ruby, secret_key: RString, sealed: RString) -> Result<RString, Error> {
    let key = secret_key_from_bytes(ruby, &secret_key.to_bytes())?;
