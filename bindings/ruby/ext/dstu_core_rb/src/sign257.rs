...
//! (`r || s`). No curve-tag byte here - distinct method names (`sign257_*`) are the whole dispatch
...
fn signing_key_from_bytes(ruby: &Ruby, bytes: &[u8]) -> Result<SigningKey, Error> {
    let d = to_array::<33>(ruby, bytes, "signing_key")?;
...
/// rejection sampling.
pub fn sign257_keygen(ruby: &Ruby) -> Result<RString, Error> {
...
pub fn sign257_verifying_key(ruby: &Ruby, signing_key: RString) -> Result<RString, Error> {
...
pub fn sign257_verify(
    ruby: &Ruby,
    verifying_key: RString,
    message: RString,
    signature: RString,
) -> Result<bool, Error> {
    let verifying_key_bytes = to_array::<66>(ruby, &verifying_key.to_bytes(), "verifying_key")?;
    let key = VerifyingKey::from_uncompressed_bytes(&verifying_key_bytes);
    let sig_bytes = to_array::<66>(ruby, &signature.to_bytes(), "signature")?;
