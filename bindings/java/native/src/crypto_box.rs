...
//! alone is a reserved Rust keyword. Keys/sealed blobs cross the boundary as plain `byte[]`,
...
//! type would buy nothing extra here (same reasoning `secretbox.rs`'s own module doc already
//! recorded). The Java-side class is named `Box` (no `crypto_` prefix, no underscore - JNI encodes
//! `_` as `_1` in generated symbol names, this binding's own standing convention, `lib.rs`'s doc
//! comment).
...
#[no_mangle]
pub extern "system" fn Java_ua_dstucrypto_dstucore_Box_keygen<'local>(
...
/// compression).
#[no_mangle]
pub extern "system" fn Java_ua_dstucrypto_dstucore_Box_publicKey<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
...
#[no_mangle]
pub extern "system" fn Java_ua_dstucrypto_dstucore_Box_seal<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
...
/// be valid.
#[no_mangle]
pub extern "system" fn Java_ua_dstucrypto_dstucore_Box_open<'local>(
    mut env: JNIEnv<'local>,
    _class: JClass<'local>,
    secret_key: JByteArray<'local>,
