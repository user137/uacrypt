...
- `crates/dstu-core` — the library (`std`/`alloc` feature flags per D-01). `dstu_core::hazmat` has
  ...
  against the official DSTU 8845:2019 text itself, see D-15; also independently confirmed against
  two state-sourced supplementary vectors from Держспецзв'язку/ДНДІ ТКЗІ (reference number and
  date deliberately not recorded, see D-104), an upgrade over UAPKI alone but still not equivalent
  to the paid standard text itself, see D-104).
