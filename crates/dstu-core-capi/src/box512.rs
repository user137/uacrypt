//! `crypto_box512` C ABI (`dstu_core::crypto_box512`) - `l(p)=512` (E512/1) sibling of
//! [`crate::crypto_box`] (T-193). Named `box512.rs`, `dstu_box512_*` - keeping `crypto_box.rs`'s
//! own dropped-`crypto_`-prefix convention (`box` alone is the reserved-keyword reason that file
//! gives; `box512` isn't a keyword, but matching the sibling's file/symbol shape is more valuable
//! here than the letter of that one justification, and every other capi module already drops the
//! prefix regardless of whether it needs to).
//!
//! **`DSTU_ERR_TAG_MISMATCH` is reused for [`OpenError::InvalidCiphertext`]**, same reasoning as
//! [`crate::crypto_box`]'s own top doc comment - `dstu_core::crypto_box512::OpenError` already
//! collapses wrong-key/tampered-wire-segment/bad-recovered-seed-length into one variant, this layer
//! must not reopen that distinction.

use crate::error::DstuStatus;
use crate::util::{guard_ptr, guard_status, guard_void, slice_from_raw, slice_from_raw_mut};
use dstu_core::crypto_box512::{open, seal, OpenError, PublicKey, SealError, SecretKey};
...
pub struct DstuBox512SecretKey(SecretKey);

/// Opaque `crypto_box512` public-key handle.
pub struct DstuBox512PublicKey(PublicKey);
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_secretkey_generate(
    out: *mut *mut DstuBox512SecretKey,
) -> DstuStatus {
    guard_status(|| {
        if out.is_null() {
        ...
        match SecretKey::generate() {
            Ok(key) => {
                unsafe { *out = Box::into_raw(Box::new(DstuBox512SecretKey(key))) };
                DstuStatus::DSTU_OK
            }
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_secretkey_from_bytes(
    bytes: *const u8,
    out: *mut *mut DstuBox512SecretKey,
) -> DstuStatus {
    guard_status(|| {
        ...
        let mut arr = [0u8; DSTU_BOX512_SECRETKEY_BYTES];
        arr.copy_from_slice(unsafe {
            std::slice::from_raw_parts(bytes, DSTU_BOX512_SECRETKEY_BYTES)
        });
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_secretkey_bytes(
    key: *const DstuBox512SecretKey,
    out: *mut u8,
) {
    guard_void(|| {
        ...
        let out = unsafe { std::slice::from_raw_parts_mut(out, DSTU_BOX512_SECRETKEY_BYTES) };
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_secretkey_public_key(
    key: *const DstuBox512SecretKey,
) -> *mut DstuBox512PublicKey {
    guard_ptr(|| {
        if key.is_null() {
            return std::ptr::null_mut();
        }
        let key = unsafe { &*key };
        Box::into_raw(Box::new(DstuBox512PublicKey(key.0.public_key())))
    })
}
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_publickey_from_bytes(
    bytes: *const u8,
    out: *mut *mut DstuBox512PublicKey,
) -> DstuStatus {
    guard_status(|| {
        if bytes.is_null() || out.is_null() {
            return DstuStatus::DSTU_ERR_NULL_POINTER;
        }
        let mut arr = [0u8; DSTU_BOX512_PUBLICKEY_BYTES];
        arr.copy_from_slice(unsafe {
            std::slice::from_raw_parts(bytes, DSTU_BOX512_PUBLICKEY_BYTES)
        });
        match PublicKey::from_bytes(&arr) {
            Some(key) => {
                unsafe { *out = Box::into_raw(Box::new(DstuBox512PublicKey(key))) };
                DstuStatus::DSTU_OK
            }
            None => DstuStatus::DSTU_ERR_INVALID_KEY,
        }
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_publickey_bytes(
    key: *const DstuBox512PublicKey,
    out: *mut u8,
) {
    guard_void(|| {
        if key.is_null() || out.is_null() {
        ...
        let key = unsafe { &*key };
        let bytes = key.0.to_bytes();
        let out = unsafe { std::slice::from_raw_parts_mut(out, DSTU_BOX512_PUBLICKEY_BYTES) };
        out.copy_from_slice(&bytes);
    })
}
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_seal(
    recipient: *const DstuBox512PublicKey,
    message: *const u8,
    message_len: usize,
    sealed_out: *mut u8,
    ...
    guard_status(|| {
        ...
        let required = message_len + DSTU_BOX512_SEAL_OVERHEAD;
...
#[no_mangle]
pub unsafe extern "C" fn dstu_box512_open(
    secret: *const DstuBox512SecretKey,
    sealed: *const u8,
    sealed_len: usize,
    ...
    guard_status(|| {
        ...
        if sealed_len < DSTU_BOX512_SEAL_OVERHEAD {
            return DstuStatus::DSTU_ERR_TRUNCATED;
        }
        let required = sealed_len - DSTU_BOX512_SEAL_OVERHEAD;
